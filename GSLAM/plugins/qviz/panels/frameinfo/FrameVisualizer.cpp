...
{
//    GSLAM::ScopedTimer tm("InfomationViewer::update");
    ...
    if(rowCount()!=vars.size())
    {
        ...
        {
            item(i,1)->setText(vars[item(i,0)->text()]);
        }
    }
}
...
void FrameVisualizer::slotFrameUpdated()
{
    ...
    for(int camIdx=0;camIdx<_curFrame->cameraNum();camIdx++)
    {
        ...
        {
            QImage qimage=QImage(img.data,img.cols,img.rows,QImage::Format_RGB888).rgbSwapped();
            ((QImageWidget*)gimageW)->setImage(qimage,true);

        }
        else if((channelFlags&IMAGE_RGBA)&&img.channels()==4)
        {
            QImage qimage=QImage(img.data,img.cols,img.rows,QImage::Format_ARGB32).rgbSwapped();
            ((QImageWidget*)gimageW)->setImage(qimage,true);
        }
...
GSLAM_REGISTER_PANEL(frameinfo,FrameVisualizer);
