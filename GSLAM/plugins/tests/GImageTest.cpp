...
{
    GSLAM::Timer& timer=GSLAM::Timer::instance();
...
void readWriteThread(GSLAM::GImage& image,int& shouldStop)
{
    ...
    std::uniform_int_distribution<int> operate(0,4);
    std::uniform_real_distribution<double> sleepSeconds(0.0001,0.0002);
    while(!shouldStop)
    {
        GSLAM::Rate::sleep(sleepSeconds(e));
        switch (operate(e)) {
        case 0:
        {
