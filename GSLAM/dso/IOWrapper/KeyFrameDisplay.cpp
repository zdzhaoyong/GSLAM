...
bool KeyFrameDisplay::refreshPC(bool canRefresh, float scaledTH, float absTH, int mode, float minBS, int sparsity)
{
    ...
    if(numGLBufferGoodPoints > numGLBufferPoints)
    {
        ...
        glBufferData(GL_ARRAY_BUFFER,sizeof(float)*3*numGLBufferPoints,tmpVertexBuffer,GL_DYNAMIC_DRAW);
        glBindBuffer(GL_ARRAY_BUFFER,colorBuffer);
        glBufferData(GL_ARRAY_BUFFER,sizeof(unsigned char)*3*numGLBufferPoints,tmpColorBuffer,GL_DYNAMIC_DRAW);
...
//        colorBuffer.Reinitialise(pangolin::GlArrayBuffer, numGLBufferPoints, GL_UNSIGNED_BYTE, 3, GL_DYNAMIC_DRAW );
    ...
    {
        glBindBuffer(GL_ARRAY_BUFFER,vertexBuffer);
        glBufferSubData(GL_ARRAY_BUFFER,0,sizeof(float)*3*numGLBufferGoodPoints,tmpVertexBuffer);
        glBindBuffer(GL_ARRAY_BUFFER,colorBuffer);
        glBufferSubData(GL_ARRAY_BUFFER,0,sizeof(unsigned char)*3*numGLBufferGoodPoints,tmpColorBuffer);
    }
