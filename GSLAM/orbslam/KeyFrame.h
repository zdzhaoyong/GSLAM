...
public:
    ...
    void setBadTS(double ts) { tsBad = ts; }
    double getBadTS(void) { return tsBad; }
...
public:
    ...
    long unsigned int mnLoopQuery;                          ///< current loop-closing query frame's ID
    int mnLoopWords;                                        ///< same word number (have the same word number with query's frame)
    float mLoopScore;                                       ///< BoW similarity with query's frame
