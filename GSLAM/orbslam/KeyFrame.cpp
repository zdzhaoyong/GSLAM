...
{
    mnId = nNextId++;                   // FIXME: change to ++nNextId ?
...
void KeyFrame::ComputeBoW()
{
    ...
    {
        vector<ORB_Descriptor> vCurrentDesc = Converter::toDescriptorVector(mDescriptors);
...
void KeyFrame::EraseMapPoint(MapPoint *pMP)
{
    ...
    for(std::vector<MapPoint*>::iterator it = mvpMapPoints.begin();
        it != mvpMapPoints.end(); it++) {

        if( *it == pMP ) {
//            dbg_pt("Found MapPoint: %8d at KF[%8d], and erase it from KeyFrame",
//                   pMP->mnId, mnId);
...
{   
    ...
    // FIXME: do not really delete the keyframe, need automatically delete
    //dbg_pt("delete keyframe: %d", this->mnId);
...
void KeyFrame::draw()
{
    ...
    double depth=ComputeSceneMedianDepth()/10.;

    Point3f t=pose.get_translation();

    Camera cam_out=SvarWithType<Camera>::instance()["CameraOut"];
