...
bool LoopClosing::DetectLoop()
{
    ...
    if( 0 ) {
//        dbg_pt("minScore = %f, vpConnectedKeyFrames.size() = %d, vpCandidateKFs.size() = %d",
//               minScore, vpConnectedKeyFrames.size(), vpCandidateKFs.size());
//        printf("KF->id = %d ---> ", mpCurrentKF->mnId);
//        for(int i=0; i<vpCandidateKFs.size(); i++) printf("%d ", vpCandidateKFs[i]->mnId);
//        printf("\n");
    }
    ...
    {
//        dbg_pt("mvpEnoughConsistentCandidates = %d", mvpEnoughConsistentCandidates.size());
...
bool LoopClosing::ComputeSim3()
{
    ...
    vpSim3Solvers.resize(nInitialCandidates, NULL);
...
//    dbg_pt("nTotalMatches = %d", nTotalMatches);
...
void LoopClosing::CorrectLoop()
{
    ...
    // Retrive keyframes connected to the current keyframe and compute
    //  corrected Sim3 pose by propagation
    ...
    // After the MapPoint fusion, new links in the covisibility graph
    //  will appear attaching both sides of the loop
    ...
    Optimizer::OptimizeEssentialGraph(mpMap, mpMatchedKF, mpCurrentKF,  mg2oScw,
