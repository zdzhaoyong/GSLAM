    ...
    mbReseting(false), mbForceRelocalisation(false), mbMotionModel(false)
{
    ...
    float fps = svar.GetDouble("Video.FPS", 60);
    if(fps==0) fps=30;
    ...
    int nRGB = svar.GetInt("Video.RGB", 1);
    mbRGB = nRGB;
    ...
    // Load ORB parameters
    int nFeatures       = svar.GetInt("ORBextractor.nFeatures", 1000);
    float fScaleFactor  = svar.GetDouble("ORBextractor.scaleFactor", 1.2);
    int nLevels         = svar.GetInt("ORBextractor.nLevels", 8);
    int fastTh          = svar.GetInt("ORBextractor.fastTh", 20);
    int Score           = svar.GetInt("ORBextractor.nScoreType", 1);
    ...
    int nMotion = svar.GetInt("ORB_SLAM.UseMotionModel", 1);
...
bool Tracking::track(GSLAM::FramePtr& videoframe)
{
    ...
    if(mK.empty())
    {
        ...
        GSLAM::Camera camera=videoframe->getCamera();
        cout<<"Using Camera:"<<camera.info()<<endl;
        VecParament<double> parameters=camera.getParameters();
        if(parameters.size()==11||parameters.size()==6)
        {
            cv::Mat K = cv::Mat::eye(3,3,CV_32F);
            K.at<float>(0,0) = parameters[2];
            K.at<float>(1,1) = parameters[3];
            K.at<float>(0,2) = parameters[4];
            K.at<float>(1,2) = parameters[5];
            K.copyTo(mK);

            cv::Mat DistCoef(5,1,CV_32F);
            if(camera.CameraType()=="OpenCV"&&parameters.size()==11)
            {
                DistCoef.at<float>(0) = parameters[6];
                DistCoef.at<float>(1) = parameters[7];
                DistCoef.at<float>(2) = parameters[8];
                DistCoef.at<float>(3) = parameters[9];
        ...
        {
            cerr<<"ORBSLAM need a OpenCV or PinHole camera.\n";
...
void Tracking::track(const cv::Mat &img_in, double timestamp)
{
    cv::Mat im;

    // convert color to gray if necessary
    if( img_in.channels() == 3 ) {
        if(mbRGB)
            cvtColor(img_in, im, CV_RGB2GRAY);
        else
            cvtColor(img_in, im, CV_BGR2GRAY);
    ...
    if( mState==WORKING || mState==LOST )
        ...
        mCurrentFrame = Frame(im, timestamp, mpIniORBextractor, mpORBVocabulary, mK, mDistCoef);
...
bool Tracking::TrackPreviousFrame()
{
    ...
    // If enough correspondeces, optimize pose and project points
    //  from previous frame to search more correspondences
    if(nmatches>=10)
    {
...
void Tracking::UpdateReferencePoints()
{
    ...
    for(vector<KeyFrame*>::iterator itKF=mvpLocalKeyFrames.begin(), itEndKF=mvpLocalKeyFrames.end();
        itKF!=itEndKF; itKF++)
    {
        KeyFrame* pKF = *itKF;
...
void Tracking::UpdateReferenceKeyFrames()
{
    ...
    for(size_t i=0, iend=mCurrentFrame.mvpMapPoints.size(); i<iend;i++)
    {
        if(mCurrentFrame.mvpMapPoints[i])
        {
            ...
            if(!pMP->isBad())
            {
                ...
                for(map<KeyFrame*,size_t>::iterator it=observations.begin(), itend=observations.end();
                    it!=itend; it++)
    ...
    for(map<KeyFrame*,int>::iterator it=keyframeCounter.begin(), itEnd=keyframeCounter.end();
        it!=itEnd; it++)
    {
        ...
        itKF!=itEndKF; itKF++)
    {
        ...
        for(vector<KeyFrame*>::iterator itNeighKF=vNeighs.begin(), itEndNeighKF=vNeighs.end();
...
bool Tracking::Relocalisation()
{
    ...
    {
        PnPsolver* pSolver = vpPnPsolvers[i];
        if( pSolver != NULL ) delete pSolver;
    }
...
//    pi::timer.enter("Tracking::Draw_Something");
    ...
    if(svar.GetInt("FrameTracker.DrawCurrentImage",1))
    {
        cv::Mat img2show = Frame2Draw.im;
        if(!img2show.empty())
        {
            double scale=svar.GetDouble("FrameTracker.ShowFrameScale",0.5);
            if( scale < 1.0 )
                cv::resize(img2show,img2show,cv::Size(img2show.cols*scale,img2show.rows*scale));

            cv::cvtColor(img2show,img2show,CV_GRAY2BGR);
            if( img2show.cols<960 && img2show.rows<540 ) {
...
//    timer.leave("Tracking::Draw_Something");
}
