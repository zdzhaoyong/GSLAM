...
public:
   ...
   */
    ...
    virtual void saveFast(const std::string &name);

    virtual void loadFast(const std::string &name);
...
protected:
     ...
     * Empty constructor
     */
        Node(): id(0), weight(0), parent(0), childNum(0),word_id(0){}

        /**
     * Constructor
     * @param _id node id
     */
        Node(NodeId _id): id(_id), weight(0), parent(0), childNum(0),word_id(0){}
     ...
     */
        ...
        bool addChild(NodeId _id)
        {
            ...
            //        if(id==0)
            //        {
            //            cout<<"Adding child "<<_id<<" for root.\n";
            //        }
            child[childNum]=_id;
            childNum++;
            return true;
                                                     ...
                                                     const vector<pDescriptor> &descriptors, int current_level)
{
    ...
    if(current_level < m_L)
    {
        ...
        NodeId* children=(NodeId*)m_nodes[parent_id].child;
        int     childNum=m_nodes[parent_id].childNum;
...
{ 
    ...
    {
        ++current_level;
        nodes =(NodeId*) m_nodes[final_id].child;
        nodeNum=m_nodes[final_id].childNum;
        final_id = nodes[0];
                                              ...
                                              const std::string &name) const
{
    ...
    while(!parents.empty())
    {
        ...
        children = (NodeId*)parent.child;
        childNum = parent.childNum;
...
template<class TDescriptor, class F>
void TemplatedVocabulary<TDescriptor,F>::saveFast(const std::string &name)
{
    ...
    // tree
    unsigned int nodeNum=m_nodes.size();
    ofs.write((char*)&nodeNum,sizeof(nodeNum));
    ofs.write((char*)m_nodes.data(),sizeof(Node)*nodeNum);

    // words
    unsigned int wordNum=m_words.size();
    ofs.write((char*)&wordNum,sizeof(wordNum));
    ...
    for(wit = m_words.begin(); wit != m_words.end(); wit++)
    {
        ...
        ofs.write((char*)&(*wit)->id,sizeof(NodeId));
    }

}

template<class TDescriptor, class F>
void TemplatedVocabulary<TDescriptor,F>::loadFast(const std::string &name)
{
    m_words.clear();
    m_nodes.clear();
    ...
    if(nodeNum>10000000){cout<<"Node number to large!\n"; exit(0);}
    m_nodes.resize(nodeNum);
    ifs.read((char*)m_nodes.data(),sizeof(Node)*nodeNum);

    // words
    unsigned int wordNum;
    ifs.read((char*)&wordNum,sizeof(wordNum));
    m_words.resize(wordNum);
    ...
    {
        ifs.read((char*)&wid,sizeof(wid));
        ifs.read((char*)&nid,sizeof(nid));
        m_nodes[nid].word_id = wid;
        m_words[wid] = &m_nodes[nid];
    }

    cout << "Loading vocabulary, mK:"<<m_k<<", mL:"<<m_L
         <<", Score:"<<m_scoring<<", Weight"<<m_weighting<<endl;
    cout << " loaded "<<m_words.size()<<" words, "<<m_nodes.size()<<" nodes.\n";
}
                                              ...
                                              const std::string &name)
{
    ...
    cout<<"loaded "<<fn.size()<<" words."<<m_nodes.size()<<" nodes.\n";
    //  cout<<"root has "<<m_nodes[0].childNum<<" children.";
    //  for(int i=0;i<m_nodes[0].childNum;i++)
    //      cout<<" "<<m_nodes[0].child[i];
}
