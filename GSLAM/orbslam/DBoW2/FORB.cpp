  ...
  FORB::TDescriptor &mean)
{
  ...
  else
  {
    ...
    {
      const FORB::TDescriptor &d = *descriptors[i];
    ...
    mean.fromMat(cv::Mat::zeros(1, FORB::L, CV_8U));
    unsigned char *p = mean.data;//mean.ptr<unsigned char>();
...
int FORB::distance(const cv::Mat &a, const cv::Mat &b)
{
    return distance(*(FORB::TDescriptor*)&a,*(FORB::TDescriptor*)&b);
}
...
{
  stringstream ss;
  const unsigned char *p = (unsigned char*)a.data;
...
{
//    return;
//    uint8_t *des = new uint8_t[FORB::L];
//    return;
  ...
  cv::Mat &mat)
{
  ...
  {
    const int C = L;//descriptors[i].cols;
    const unsigned char *desc = descriptors[i].data;
  ...
  cv::Mat &mat)
{
  ...
  {
    const unsigned char *d = descriptors[i].data;
    std::copy(d, d+32, p);
  }
